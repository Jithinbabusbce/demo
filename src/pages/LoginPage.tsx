...
export default function LoginPage({ onLogin }: { onLogin?: (name: string) => void }) {
  ...
  return (
    <div className={`subpage ${styles.loginPage}`}>
      <div className="subpage-content">
        <div className={styles.loginCard}>
          <h2>Sign in</h2>
          <p>Choose a login method, get OTP, and continue.</p>

          <div className={styles.loginAuthToggle} aria-label="Social login options">
            <button type="button" className={`${styles.socialLoginBtn}${provider === 'google' ? ' selected' : ''}`} onClick={() => chooseProvider('google')}>
              <span className={`${styles.socialIcon} google`} aria-hidden="true">G</span>
              <span>Continue with Gmail</span>
            </button>
            <button type="button" className={`${styles.socialLoginBtn} facebook${provider === 'facebook' ? ' selected' : ''}`} onClick={() => chooseProvider('facebook')}>
              <span className={`${styles.socialIcon} facebook`} aria-hidden="true">f</span>
              <span>Continue with Facebook</span>
            </button>
          </div>

          <div className={styles.loginOrDivider}>
          ...
          >
            ...
            {otpSent ? (
              <>
                ...
                />
                <div className={styles.loginOtpRow}>
          ...
          <div aria-live="polite" aria-atomic="true">
            {statusMessage ? (
              <p className={`${styles.loginStatus} ${statusTone}`} role="status">{statusMessage}</p>
            ) : null}
          </div>
