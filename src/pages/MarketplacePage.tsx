...
export default function MarketplacePage() {
  ...
  return (
    <div className="subpage">
      <div className="subpage-hero marketplace-hero">
      ...
      <div className="subpage-content">
        <section className="marketplace-mode-panel">
          <button type="button" className={`mode-card ${mode === 'hire' ? 'active' : ''}`} onClick={() => setMode('hire')}>
            <h3>I'm looking to hire</h3>
            <p>My team needs vetted sports talent and services.</p>
          </button>
          <button type="button" className={`mode-card ${mode === 'offer' ? 'active' : ''}`} onClick={() => setMode('offer')}>
        ...
        <p className="marketplace-results-meta">
          {filteredListings.length} results found
          {mode === 'hire' ? ' for hiring needs' : ' for seller profiles'}
        </p>

        <div className="card-list marketplace-grid">
          {filteredListings.map((card) => (
            <article className="info-card marketplace-card" key={card.id}>
